...
def process_COMMAND_BL_GET_VER(length):
    ver=read_serial_port(2)
    value = bytearray(ver)
    print("   Version Major:", value[0])
    print("   Version Minor:", value[1])
...
def decode_menu_command_code(command):
    ...
    if(command  == 0 ):
        print("\n   Exiting...!")
        raise SystemExit
    elif(command == 1):#LENGTH + COMMAND CODE
        print("\n   Command == > BL_GET_VER")
        COMMAND_BL_GET_VER_LEN              = 2
        data_buf[0] = COMMAND_BL_GET_VER_LEN-1 
        data_buf[1] = COMMAND_BL_GET_VER 
##        crc32       = get_crc_stm32_style(data_buf,COMMAND_BL_GET_VER_LEN-4)
...
##        data_buf[5] = word_to_byte(crc32,4,1) 
        ...
        for i in data_buf[0:COMMAND_BL_GET_VER_LEN]:
        ...
        #==========================
        Write_to_serial_port(1,1) #dummy sent
        ack=read_serial_port(1)
        a_array=bytearray(ack)
        if(a_array[0] == 0x01):
            print("\nCurrently running FW active version is 1")
            print("Enter traget address 0x08040000 to flash in slot 2\n")
        elif(a_array[0] == 0x02):
            print("\nCurrently running FW active version is 2")
            print("Enter traget address 0x08008000 to flash in slot 1\n")
        elif(a_array[0] == 0x0):
            print("\nboth slots empty, anyone can be programmed\n")
        #==========================
